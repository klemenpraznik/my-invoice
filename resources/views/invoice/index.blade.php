    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card card-custom">
                ...
                <div class="card-body">
                    <table id="invoices" class="hover table table-bordered dt-responsive nowrap" width="100%" cellspacing="0">
                        <thead>
                            <tr>
                                <th scope="col">Št. računa</th>
                                <th scope="col" data-priority="1">Stranka</th>
                                <th scope="col">Tip</th>
                                <th scope="col">Skupaj brez DDV</th>
                                <th scope="col">Znesek brez DDV:</th>
                                <th scope="col">Znesek z DDV:</th>
...
<script>
    $(document).ready(function () {
        var ime = @json(Auth::user()->name);
        console.log(ime);
        $.noConflict();
        var table = $("#invoices").DataTable({
            ajax: {
                headers: { "api_token" : "{{auth()->user()->api_token}}" },
                url: "/api/user/{{auth()->user()->id}}/invoices",
                dataSrc: ""
            },
            columns: [
                {
                ...
                bootbox.confirm("Ali ste pripričani, da želite izbrisati ta izdelek?",
                    function (result) {
                        if (result) {
                            $.ajax({
                                headers: { "api_token" : "{{auth()->user()->api_token}}" },
                                url: "/api/user/{{auth()->user()->id}}/invoice/" + button.attr("data-category-id"),
                                method: "DELETE",
                                success: function () {
